...
public:
    virtual stream_t  yield_streams
...
protected:                      // Types
    ...
    using p_cb_t = std::shared_ptr< func_t >;
    using f_cb_t = void ( * )   ( void   * );
protected:                      // Data
    std::map< stream_t, std::vector< p_cb_t > > cb_buf;
protected:                      // Function
    static  void callback_std  ( void           * p ) ;
    virtual void callback_clear( const stream_t & s ) ;
public:                         // Function
    virtual void  f_launch_host
    ( const stream_t & s, const  f_cb_t f, void * p ) = 0;
    virtual void    launch_host( const stream_t & s ,
                                 const   func_t & f ) ;
};
...
__forceinline__ void base_t::__launch
( const fun_T & ker,    const dim3   &     n_bl ,
  const dim3  & n_th,   const int    &     s_sh ,
  const void  * stream, const args_T & ... args ) const
{
