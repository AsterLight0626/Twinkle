...
{
    return ( * reinterpret_cast< func_t * >( p ) ) (  );
}

void base_t::callback_clear( const stream_t & stream )
{
    return cb_buf[ stream ].clear(  );
}

void base_t::launch_host( const stream_t & s ,
                          const   func_t & f )
{
    auto & p = cb_buf[ s ].emplace_back(  );
    p  = std::make_shared< func_t >    (  );
    (  * p ) = f;
    return f_launch_host( s, callback_std, p.get(  ) ) ;
}
