...
__device__ bool cmplx_roots_gen( img_T *imgs, c_T *poly, int degree, bool polish_roots_after, bool use_roots_as_starting_points)
{
	...
	if (degree <= 1) {
		...
		if(degree == 1)
		{
			imgs[0].position.set(-poly[0].re, -poly[0].im);
			imgs[0].position /= poly[1];
		}
		return false;
	}
	...
	for (int n = degree; n >= 3; n--) {
		...
		for (int i = n - 1; i >= 0; i--) {
			...
			coef *= imgs[n - 1].position; coef += prev;
		}
	}
	...
	{
		sum += imgs[j].position;
		multi *= imgs[j].position;
	}

	c_T vieta_temp;
	if constexpr ( std::is_same_v< f_T, double > )
	{
		vieta_temp = poly[0]; vieta_temp /= poly[degree];
		if((degree & 1)!=1){ vieta_temp *= -1;}
		vieta_temp += multi;
		failed = norm(vieta_temp) < 1e-10;

		vieta_temp = poly[degree-1]; vieta_temp /= poly[degree]; vieta_temp += sum;
		failed = !(norm(vieta_temp)<1e-10  && failed);
	}
	else
	{
        if constexpr ( std::is_same_v< f_T, float > )
		{
			...
			if((degree & 1)!=1){ vieta_temp *= -1;}
			vieta_temp += multi;
			failed = norm(vieta_temp) < 1e-5;

			vieta_temp = poly[degree-1]; vieta_temp /= poly[degree]; vieta_temp += sum;
			failed = !(norm(vieta_temp)<1e-5  && failed);
		}
		else{failed = true;}		
	}
...
__device__ void solve_quadratic_eq(c_T &x0, c_T &x1, c_T *poly)
{
	...
	temp = a; temp *= c; temp *= 4;
	delta = b; delta *= delta; delta -= temp;
	delta = sqrt(delta); 
	...
	for (i = 1; i <= MAXIT; i++) {
		...
		if ((i&7) == 1) { //calculate stopping criterion every tenth iteration
			...
			for (k = degree - 1; k >= 0; k--) {
				...
				dp *= *root; dp += p;
				// p = poly[k] + p * (*root); //b_k
				p *= *root;				   //Adams, Duane A., 1967, "A stopping criterion for polynomial root finding",
				p += poly[k];			   //Communications of ACM, Volume 10 Issue 10, Oct. 1967, p. 655
		...
		if (abs2p < stopping_crit2) { //simplified a little Eq. 10 of Adams 1967
			if (dp == 0) return; //if we have problem with zero, but we are close to the root, just accept
...
__device__ void cmplx_laguerre(c_T *poly, int degree, c_T *root, int &iter, bool &success)
{
	...
	f_T ek, absroot, abs2p;
	c_T fac_newton, fac_extra, F_half;
	...
	for (i = 1; i <= MAXIT; i++) {
		...
		for (k = degree - 1; k >= 0; k--) {
			...
			d2p_half *= *root; d2p_half += dp;
			// dp = p + dp * (*root);
			dp *= *root; dp += p;
			// p = poly[k] + p*(*root); // b_k
			p *= *root;				 //Adams, Duane A., 1967, "A stopping criterion for polynomial root finding",
			p += poly[k];			 //Communications of the ACM, Volume 10 Issue 10, Oct. 1967, p. 655
									 //ftp://reports.stanford.edu/pub/cstr/reports/cs/tr/67/55/CS-TR-67-55.pdf
									 //Eq 8.
			ek = absroot*ek + abs_c(p);
		}
		...
		if (dp != 0) {
			// fac_newton = p / dp;
			fac_newton = p; fac_newton /= dp;
			// fac_extra = d2p_half / dp;
			fac_extra = d2p_half; fac_extra /= dp;
			// F_half = fac_newton*fac_extra;
			F_half = fac_newton; F_half *= fac_extra;
			// denom_sqrt = sqrt(1 - two_n_div_n_1*F_half);
			denom_sqrt = F_half;
			denom_sqrt *= -two_n_div_n_1;
			denom_sqrt += 1;
			denom_sqrt = sqrt(denom_sqrt);
		...
		if (denom == 0) {
			...
			// dx = fac_newton / denom;
			dx = fac_newton; dx /= denom;
		}


		// newroot = *root - dx;
		newroot = *root; newroot -= dx;
...
__device__ void cmplx_laguerre2newton(c_T *poly, int degree, c_T *root, int &iter, bool &success, int starting_mode)
{
	...
	c_T fac_netwon, fac_extra, F_half;
	f_T one_nth, n_1_nth, two_n_div_n_1;
	int mode;
	f_T stopping_crit2;
		...
		if (mode >= 2) {//Laguerre's method
			...
			for (i = 1; i <= MAXIT; i++) {
				...
				for (k = degree; k >= 1; k--) {//Horner Scheme, see for eg.  Numerical Recipes Sec. 5.3 how to evaluate polynomials and derivatives
					...
					p *= (*root);				   //Adams, Duane A., 1967, "A stopping criterion for polynomial root finding",
					p += poly[k - 1];		 	   //Communications of the ACM, Volume 10 Issue 10, Oct. 1967, p. 655
												   //ftp://reports.stanford.edu/pub/cstr/reports/cs/tr/67/55/CS-TR-67-55.pdf
												   //Eq 8.
					ek = absroot * ek + abs_c(p);
				}
				...
				if (dp != 0) {
					...
					// denom_sqrt = sqrt(1 - two_n_div_n_1*F_half);	// sqrt part in delta_Lag
					denom_sqrt = F_half;
					denom_sqrt *= -two_n_div_n_1;
					denom_sqrt += 1;
					denom_sqrt = sqrt(denom_sqrt);
		...
		if (mode == 1) {//SECOND-ORDER GENERAL METHOD (SG)
			...
			for (i = j; i <= MAXIT; i++) {
				...
				if (dp == 0) {//test if denominators are > 0.0 not to divide by zero
					...
					// fac_netwon = p / dp;
					fac_netwon = p; fac_netwon /= dp;
					// fac_extra = d2p_half / dp;
					fac_extra = d2p_half; fac_extra /= dp;
					// F_half = fac_netwon * fac_extra;
					F_half = fac_netwon; F_half *= fac_extra;

					abs2_F_half = norm(F_half);
					if (abs2_F_half <= 0.000625f) {//F<0.05, F/2<0.025
						mode = 0; //set Newton's, go there after jump
					}
					// dx = fac_netwon * (1 + F_half); //SG
					dx = F_half; dx += 1; dx *= fac_netwon;
				}
				// newroot = *root - dx;
				newroot = *root; newroot -= dx;
				if (newroot == *root) return; //nothing changes -> return
